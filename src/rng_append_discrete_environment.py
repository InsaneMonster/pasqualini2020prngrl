...
from simulator.rng_simulator_append_discrete import RNGSimulatorAppendDiscrete


class RNGAppendDiscreteEnvironment(Environment):
    """
    Base environment class for the RNG Append Discrete task.
    """
    ...
    def __init__(self,
        ...
        super(RNGAppendDiscreteEnvironment, self).__init__(name)
    ...
    def _generate(self,
        ...
        for i in range(self._parallel):
            self._rng_simulators.append(RNGSimulatorAppendDiscrete(self._name + "_" + str(i), self._append_sequence_size, self._max_moves, self._simulator_can_render))
        # Setup attributes
    ...
    def step(self,
        ...
        for i in range(len(self._rng_simulators)):
            ...
            _, score, episode_done = self._rng_simulators[i].step(action[i])
            state_next = self._rng_simulators[i].last_appended_sequence
            # Compute the reward from the score
            reward: float = score * self._lambda_parameter
            # Save results (take only the last appended sequence as state)
    ...
    def possible_actions(self,
        ...
        for i in range(len(self._rng_simulators)):
            possible_actions.append([action for action in range(self._rng_simulators[i].append_sequences_number)])
    ...
    def state_space_shape(self) -> ():
        ...
        assert (len(self._rng_simulators) > 0)
        # Get the number of available states (continuous, so size of the append sequences) in the simulator
        return numpy.zeros(self._rng_simulators[0].append_sequence_size).shape
    ...
    def action_space_shape(self) -> ():
        ...
        return self._rng_simulators[0].append_sequences_number,
